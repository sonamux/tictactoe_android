...
import com.appspot.massive_glyph_88905.tictactoe.Tictactoe;
import com.appspot.massive_glyph_88905.tictactoe.model.TictactoeApiMessagesBoardMessage;
import com.appspot.massive_glyph_88905.tictactoe.model.TictactoeApiMessagesScoreRequestMessage;
import com.appspot.massive_glyph_88905.tictactoe.model.TictactoeApiMessagesScoreResponseMessage;
import com.appspot.massive_glyph_88905.tictactoe.model.TictactoeApiMessagesScoresListResponse;
...
public class TictactoeActivity extends Activity {
  ...
  private void getComputerMove(final String boardString) {
     new GetComputerMoveTask().execute(boardString);
  }
   ...
   */
  private class GetComputerMoveTask extends AsyncTask<String, String, String> {
    @Override
    protected String doInBackground(String... boards) {
        String newBoard = null;
      try {
        newBoard = service.board().getmove(new TictactoeApiMessagesBoardMessage().setState(boards[0])).execute().getState();
        return newBoard;
      } catch (IOException e) {
    ...
    @Override
    protected void onPostExecute(String board) {
      setBoardFilling(board);

      int status = checkForVictory(board);
      if (status != 0) {
        handleFinish(status);
      } else {
        waitingForMove = true;
  ...
  private void sendResultToServer(int status) {
    ...
    new SendResultToServerTask().execute(status);
  }
   ...
   * blocking the UI.
   */
  private class SendResultToServerTask extends AsyncTask<Integer, Void, Void> {
    @Override
    protected Void doInBackground(Integer... status) {
      try {
          service.scores().insert(new TictactoeApiMessagesScoreRequestMessage().setOutcome(STATUS_STRINGS[status[0]])).execute();
      } catch (IOException e) {
        e.printStackTrace();
      }
      return null;
   ...
   * blocking the UI.
   */
  private class QueryScoresTask extends AsyncTask<Void, Void, TictactoeApiMessagesScoresListResponse> {
    Activity activity;
    ...
    @Override
    protected TictactoeApiMessagesScoresListResponse doInBackground(Void... unused) {

      try {
          return  service.scores().list().execute();
      } catch (IOException e) {
        e.printStackTrace();
      }
      return null;
    ...
    @Override
    protected void onPostExecute(TictactoeApiMessagesScoresListResponse scores) {
      ...
      if (scores != null && scores.getItems() != null) {
        for (TictactoeApiMessagesScoreResponseMessage score : scores.getItems()) {
